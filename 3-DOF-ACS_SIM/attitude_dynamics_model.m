function [t,res] = attitude_dynamics_model(tspan,tsteps,z0,IC)
...
%T_control =@(t)[interp1(indices,transpose(IC.T_control(1,:)),t);interp1(indices,transpose(IC.T_control(2,:)));interp1(indices,transpose(IC.T_control(3,:)))]; 
[t,res] = ode45(@attitude_eom,linspace(0.001,tspan,tsteps),z0);

    function dz = attitude_eom(t,z)
        ...
        wdot = IC.MOI\(-[0,-w(3),w(2);w(3),0,-w(1);-w(2),w(1),0]*IC.MOI*w + IC.T_control(:,round(t*size(IC.T_control,2))));
        wdot1 = wdot(1);
        wdot2 = wdot(2);
        wdot3 = wdot(3);
