...
if numel(x(x > 0.00001)) > TNUM_LIM
    fmin = fmin*TNUM_PEN*numel(x(x > 0.00001));
end
...
fmin = PROP_PEN*fmin + ... % Prop usage penalty and thruster count penalty
    1e8*sum(LBND_CONSTR(LBND_CONSTR > -1e-10),2) + ... % Lower bound violation penalty
    BND_PEN*sum(UBND_CONSTR(UBND_CONSTR > 0),2) + ... % Upper bound penalty
    sum(abs(fconstr),2); % Target force and torque penalty
return
